...
public class XboxModuleManager {
    private static readonly Dictionary<Type, Func<MemoryEngine360, IConsoleConnection, XboxModuleManager, Task>> processors = new();
    ...
    public static void RegisterHandlerForConnectionType<T>(Func<MemoryEngine360, T, XboxModuleManager, Task> function) where T : IConsoleConnection {
        ArgumentNullException.ThrowIfNull(function);
        processors[typeof(T)] = (engine, connection, manager) => function(engine, (T) connection, manager);
    }

    public static async Task<bool> TryFillModuleManager(MemoryEngine360 engine, IConsoleConnection connection, XboxModuleManager manager) {
        for (Type? type = connection.GetType(); type != null; type = type.BaseType) {
            if (processors.TryGetValue(type, out Func<MemoryEngine360, IConsoleConnection, XboxModuleManager, Task>? processor)) {
