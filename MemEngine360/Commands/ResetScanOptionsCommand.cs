...
public class ResetScanOptionsCommand : BaseMemoryEngineCommand {
    ...
    protected override async Task ExecuteCommandAsync(MemoryEngine360 engine, CommandEventArgs e) {
        if (!engine.ScanningProcessor.IsScanning) {
            engine.ScanningProcessor.StartAddress = BasicApplicationConfiguration.StartAddressProperty.DefaultValue;
            engine.ScanningProcessor.ScanLength = BasicApplicationConfiguration.ScanLengthProperty.DefaultValue;
