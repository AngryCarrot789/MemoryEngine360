...
public sealed class ThreadedLuaScript {
    ...
    public ThreadedLuaScript(Script ownerScript, Chunk sourceChunk) {
    ...
    private void ScriptMain() {
        ...
        try {
            ...
            // Use normal stop cancellation token to let it exit normally
            Task<int> operation = this.luaState.RunAsync(this.sourceChunk, values, this.CancellationToken).AsTask();

            // Wait for the machine to exit normally, or force exit when user wants to kill it.
            // We can't actually kill the thread because thread.Abort() is no longer supported.
            operation.Wait(this.KillCancellationToken);

            if (!operation.IsCompleted) { // wtf?
                this.tcsCompletion.SetResult(new LuaValue[0]);
            }
            else if (operation.IsCanceled) {
                this.tcsCompletion.SetCanceled(this.CancellationToken);
            }
            else if (operation.IsFaulted) {
                this.tcsCompletion.SetException(operation.Exception);
            }
            else {
                int resultCount = operation.GetAwaiter().GetResult();
                LuaValue[] results = new LuaValue[resultCount];
                values.AsSpan(0, resultCount).CopyTo(results);
            ...
            this.tcsCompletion.SetException(exception = e);
        }
        finally {
            this.luaState = null;
            Interlocked.Exchange(ref this.initialCts, null)?.Dispose();
            Interlocked.Exchange(ref this.killCts, null)?.Dispose();
            Script.InternalOnStopped(this.ownerScript, exception);
        }
        
        this.Print("Lua Thread Exited");
    }
    ...
    private static void OpenOSLibrary(LuaState theState) {
        LuaTable luaTable = new LuaTable(0, OperatingSystemLibrary.Instance.Functions.Length);
