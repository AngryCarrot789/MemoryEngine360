...
public class SuspendThreadCommand : BaseDebuggerCommand {
    ...
    protected override async Task ExecuteCommandAsync(ConsoleDebugger debugger, CommandEventArgs e) {
        ...
        using IBusyToken? token = await debugger.BusyLock.BeginBusyOperationUsingActivity("Unfreeze Console");
        if (token != null && debugger.Connection != null) {
            if (debugger.ActiveThread == null)
                return;

            try {
                await (debugger.Connection.GetFeatureOrDefault<IFeatureXboxDebugging>()!).SuspendThread(debugger.ActiveThread.ThreadId);
