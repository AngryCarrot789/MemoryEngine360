...
public class PointerScannerV2 {
    ...
    // Virtual address (virtualBaseAddress + offset) to value
    private readonly SortedList<uint, uint> alignedVirtualAddressToValue = new SortedList<uint, uint>(200000);

    // A set of offsets (relative to virtualBaseAddress) that have a value.
    private readonly List<uint> unalignedValidOffsets = new List<uint>(50000);

    // Maps an offset (relative to virtualBaseAddress) to a set of offsets that, when added to the key, map to a value in offsetToValue
    ...
    public PointerScannerV2(MemoryEngine memoryEngine) {
    ...
    private void ScanRecursive(ref ScanningContext ctx) {
        ...
        for (int i = 0; i < unalignedOffsets.Count; i++) {
            ...
            if (virtualAddress == ctx.searchAddress) {
                ...
                uint dstAddress = this.unalignedOffsetToValue![unalignedAddressOffset + unalignedOffsets[i]];
                if (dstAddress == ctx.searchAddress) {
                    ctx.thread.IncrementTotalAddresses();
                    this.AddPointerResult(ctx.CurrentChain, alignedOffset);
                }
                else if (ctx.CurrentDepth < ctx.maxDepth) {
                    ctx.thread.IncrementTotalAddresses();
                    ctx.PushFrame(new PointerPrivate(ctx.TopPointer.value, alignedOffset, dstAddress));
                    this.ScanRecursive(ref ctx);
                    ctx.PopFrame();
                }
            }
    ...
    public async Task GenerateBasePointerMap(IActivityProgress progress, CancellationToken cancellation = default) {
        ...
        this.unalignedOffsetToValue = new uint[this.memoryDump!.Length / align];
                ...
                using (progress.CompletionState.PushCompletionRange(0, 1.0 / dumpSpan.Length)) {
                    ...
                    for (int i = 0; i < dumpSpan.Length; i += align) {
                        ...
                        if (u32value != 0 && this.addressableRange.Contains(u32value)) {
                            ...
                            uint unalignedOffset = (uint) i / (uint) align;
                            this.unalignedValidOffsets.Add(unalignedOffset);
                            this.unalignedOffsetToValue[unalignedOffset] = u32value;
                        }
            ...
            await Task.Run(() => {
                using (progress.CompletionState.PushCompletionRange(0, 1.0 / this.unalignedValidOffsets.Count)) {
                    ...
                    foreach (uint unalignedValidOffset in this.unalignedValidOffsets) {
                        cancellation.ThrowIfCancellationRequested();
                        uint alignedOffset = unalignedValidOffset * (uint) align;
                        ...
                        try {
                            Parallel.For(this.minimumOffset / align, this.maximumOffset / align, options, (i, state) => {
                                ...
                                long absOffset = (alignedOffset + i * align) / align;
                                if (absOffset < 0 || absOffset >= unalignedValueArray.Length) {
                                    return;
                                }

                                uint value = unalignedValueArray[absOffset];
                                if (value == 0) {
                                    return;
                                }
