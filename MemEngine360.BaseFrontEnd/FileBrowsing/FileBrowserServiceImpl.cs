...
public class FileBrowserServiceImpl : IFileBrowserService {
    private WeakReference<FileTreeExplorerWindow>? currentWindow;

    public Task ShowFileBrowser(FileTreeExplorer explorer) {
        if (WindowingSystem.TryGetInstance(out WindowingSystem? system)) {
            if (this.currentWindow == null || !this.currentWindow.TryGetTarget(out FileTreeExplorerWindow? existing) || existing.IsClosed) {
                FileTreeExplorerWindow window = new FileTreeExplorerWindow() {
                    PART_FileBrowser = { FileTreeManager = explorer }
                };

                if (this.currentWindow == null)
                    this.currentWindow = new WeakReference<FileTreeExplorerWindow>(window);
                else
